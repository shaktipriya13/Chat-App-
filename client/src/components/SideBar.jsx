...
// // import { setOtherUsers, setSearchData, setSelectedUser, setUserData } from '../redux/user.slice.js';
// // import { useNavigate } from 'react-router-dom';
// // function SideBar() {
// //     let { userData, otherUsers, selectedUser, onlineUsers, searchData } = useSelector(state => state.user)
// //     let [search, setSearch] = useState(false) //search becomes active when we click on it
...
// //             /*👉 The withCredentials: true ensures:
// //                     ✅ That cookie is sent to the backend->
// //                     ✅ So the backend can verify the token->
// //                     ✅ And then log the user out (by clearing the cookie)*/
...
// //         try {
// //             let result = await axios.get(`${serverUrl}/api/v1/user/search?query=${input}`, { withCredentials: true })
...
// //                 <h1 className='text-white font-bold text-[25px]'>PingDrop</h1>
...
//     const handleSearch = async () => {
//         try {
//             const result = await axios.get(`${serverUrl}/api/v1/user/search?query=${input}`, { withCredentials: true })
//             dispatch(setSearchData(result.data))
//         } catch (error) {
